...
Temperatures are given as celsius * 100 and precipitation is given as mm * 100. For questions please contact: mathias.neumann@boku.ac.at
...
SofiaPrcp2010 <- terra::extract(ras,
                                coords,
                                list = TRUE) %>%
  unlist()
```
...
Make Bulgaria boundary a `vect` object.
```{r}
Bulgaria0.v <- vect(Bulgaria0.sf)

Bulgaria.df <- terra::extract(ras,
                              Bulgaria0.v,
                              fun = mean,
                              na.rm = TRUE)

Precip.r <- terra::crop(ras,
                        Bulgaria0.v)
plot(Precip.r)
```
